...
if x <= 1:    # If the integer entered by the user is less than or equal to one the below prompt will print to the screen and ask them to enter only a number greater than one
    print("Prime numbers are greater than one, please enter a positive integer greater than one")
if x > 1:  # Prime numbers are greater than one and so I have asked for the loop to run only for numbers greater than one
  for i in range (2, x):  # A range has been defined between 2 up to but not including the number entered by the user(x).
    if x % i == 0:      # the if statement here will run within the for loop through all the numbers starting at 2 up to but not including the number that has been inputted by the user (x) and if any of the numbers are evenly divisible into the number inputted it is not a prime number 
